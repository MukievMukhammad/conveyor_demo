...
class ConveyorCV:
	...
	def camera_callback(self, msg):
		...
		except cv_bridge.CvBridgeError as er:
			print(er)
	...
	def process_img(self, image):
		...
		for cnt in contours:
			...
			if area > 300000:
				...
				# draw details bound
				for d_box, clr in details_box:
					W = rect[1][0]
					H = rect[1][1]

					d_box[:,0] += min(box[:,0]) + int(W * 0.2 / 4 - 3)
				...
				if np.any([box[:,1] < 500 for box, clr in details_box]):
					cnvyr = Conveyor()
					cnvyr.speed = 0
					...
					d = np.array\
						([
							[int(np.mean(d_box[:,0])), int(np.mean(d_box[:,1])) , 0 if clr == "blue" else 1] 
							for d_box, clr in details_box
						])
					# next_detail_coorns = (np.array([details_box[np.argmin(d_coords[:,1])]], dtype=np.float32), )[0]
					min_ditail_coord_pxl = d[np.argmin(d[:,1])]
					next_d_box = min_ditail_coord_pxl[2]
					min_ditail_coord_pxl_cntr = [min_ditail_coord_pxl[0] - 1280/2, min_ditail_coord_pxl[1] - 1024/2]
					next_detail_coord = [i * 0.05 for i in min_ditail_coord_pxl_cntr] # tf from px to cm
					...
					# --- end tf
					try:
						resp = self.take_from_cnvyr_srv(take_coord, next_d_box)
						print("resp: ", resp)
					except:
	...
	def find_detail_on_cnvry(self, img):
		...
		for cnt in contours:	
			...
			area = int(rect[1][0]*rect[1][1]) # вычисление площади
			if area > 100:
				d_center = int(np.mean(box[:,0])), int(np.mean(box[:,1]))
				clr_hsv = img_hsv[d_center[1], d_center[0]]
				clr = "unknown"
				if clr_hsv[0] >= 88 and clr_hsv[0] <= 106 and clr_hsv[1] >= 242 and clr_hsv[2] >= 151:
					clr = "blue"
				if clr_hsv[0] >= 16 and clr_hsv[0] <= 100 and \
					clr_hsv[1] >= 26 and clr_hsv[1] <= 90 \
					and clr_hsv[2] >= 230:
					clr = "white"
				details_box.append((box, clr))
...
def main():
	...
	while not rospy.is_shutdown():
		...
		time_point = rospy.Time.now()

		while not rospy.is_shutdown() and \
			(len(conveyorCV.image_buffer) == 0 or conveyorCV.image_buffer[-1][0] < time_point):
			rospy.sleep(rospy.Duration(nsecs=10))
 
		conveyorCV.process_img(conveyorCV.image_buffer.pop()[1])
